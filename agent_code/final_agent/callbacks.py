...
def build_danger_map(game_state):
    ...
    bomb_data = {
        tuple(position): {
            'timer': float(timer),
            'blast': calculate_blast_cells(field, tuple(position))
        }
...
def nearest_coin_distance(game_state, start_position=None):
    ...
    for cx, cy in coins:
        if in_bounds(cx, cy) and field[cx, cy] != 1:
    ...
    sx, sy = start_position
    nearest_distance = int(distance_map[sx, sy]) if np.isfinite(distance_map[sx, sy]) else None
...
def extract_features_and_mask(game_state, precomputed_danger=None, precomputed_safe_tiles=None, precomputed_coin_map=None):
    ...
    if danger_time[sx, sy] > 1 and len(safe_tiles) > 1:
        mask[4] = 1.0
    if np.sum(mask[:4]) == 0:
        mask[4] = 1.0
    ...
    if (
        has_bomb
        and (targets_crate or targets_opponent)
...
def setup(self):
    ...
    if not getattr(self, "train", False):
        ...
        self.policy_net.load_state_dict(checkpoint["policy_net"] if isinstance(checkpoint, dict) and "policy_net" in checkpoint else checkpoint)
